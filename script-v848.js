(function(){
  ...
  if(showDebug){ var bar=qs('#debugBar'); if(bar) bar.classList.remove('hidden'); }
  var frb=qs('#forceResolveBtn'); if(frb) frb.addEventListener('click', function(){ requestResolve(true); });
  var ssb=qs('#showStateBtn'); if(ssb) ssb.addEventListener('click', function(){ if(!currentGameId) return; db.ref('games/'+currentGameId).get().then(function(s){ qs('#stateDump').textContent=JSON.stringify(s.val(), null, 2); }); });

  // State
  var currentGameId=null;
  var myRole=null;
  var selectStarted=false;
  var deadlineInterval=null, reconcileInterval=null, debounceTimer=null;
  ...
  // Routing iniziale
  var urlGameId=getGameId();
  var urlHostId=getHostId();
  ...
  function showCreatorStart(){
    ...
    inviteBtn.addEventListener('click', function(){
      ...
      db.ref('games/'+newId).set({
        player1:{ name:name, joined:true, clientId:CLIENT_ID, role:'p1' },
        createdAt: Date.now(),
        phase:'waiting',
        version:'8.4.8'
      });
  ...
  function showCreatorWaiting(gid){
    ...
    outcomeSection.classList.add('hidden');
    actionSection.classList.add('hidden');
    var linkOpponent=location.origin+location.pathname+'?game='+gid;
    var linkHost    =location.origin+location.pathname+'?host='+gid;
    qs('#creatorStart').classList.add('hidden');
    qs('#gameLinkWrap').classList.remove('hidden');
    qs('#gameLinkOpponent').value=linkOpponent;
    qs('#gameLinkHost').value=linkHost;
  ...
  function showInvitee(gid){
    ...
    db.ref('games/'+gid).once('value').then(function(s){
      var d=s.val();
    ...
    acceptBtn.onclick=function(){
      ...
      db.ref('games/'+gid+'/player2').set({ name:name, joined:true, clientId:CLIENT_ID, role:'p2' }).then(function(){
        ...
        db.ref('games/'+gid).once('value').then(function(s){
          var d=s.val();
          if(d && d.player1 && d.player1.joined){
            db.ref('games/'+gid+'/phase').set('select');
          }
        });
      });
    };
    ...
    for(var r=0;r<ranks.length;r++){
      for(var s=0;s<suits.length;s++){
        (function(code){
          ...
          cell.addEventListener('click', function(){ toggleCard(code,cell); });
  ...
  function updateSel(){
    ...
    var ok = (selected.length===8 || timeZero);
    qs('#confirmBtn').disabled = !ok;
  }
  function paintSelection(){
    qsa('.card').forEach(function(c){
      if(selected.indexOf(c.dataset.card)>-1) c.classList.add('selected');
  ...
  function autoComplete(){
    var deck=[]; for(var i=0;i<ranks.length;i++){ for(var j=0;j<suits.length;j++){ deck.push(ranks[i]+suits[j]); } }
    while(selected.length<8){
      var pick=deck[Math.floor(Math.random()*deck.length)];
      if(selected.indexOf(pick)===-1) selected.push(pick);
    }
  }
  function onTimeExpired(whichRole){
    if(selected.length<8){ autoComplete(); paintSelection(); }
    qs('#confirmBtn').disabled=false;
    qs('#selectInfo').textContent='Tempo scaduto: selezione completata automaticamente. Conferma per continuare.';
    lockGrid();
  }
  function startSelectCards(whichRole){
    ...
    actionSection.classList.add('hidden');
    selected=[]; buildGrid();
    var tl=60; timeZero=false; qs('#timeLeft').textContent=tl;
    if(timerInt) clearInterval(timerInt);
    timerInt=setInterval(function(){
      tl--;
      if(tl<=0){ tl=0; timeZero=true; qs('#timeLeft').textContent=tl; clearInterval(timerInt); onTimeExpired(whichRole); }
      else { qs('#timeLeft').textContent=tl; }
    }, 1000);

    qs('#confirmBtn').onclick=function(){
  ...
  function startBettingCountdown(sec){
    var el=qs('#startBettingCountdown'); if(!el) return;
    ...
    (function tick(){
      ...
      if(n<=0){
        ...
        db.ref('games/'+currentGameId).transaction(function(data){
          if(!data) return data;
  ...
  function setBetControlsEnabled(on){
    qs('#betSelect').disabled=!on; qs('#betInput').disabled=!on; qs('#betZero').disabled=!on;
    qs('#betLock').textContent = on ? '' : 'Caricamento giocatori…';
  }
  function clampBetInput(max){
    var inp=qs('#betInput'); var parsed=parseInt(inp.value,10);
    var v=isNaN(parsed)?0:parsed; var v2=Math.max(0,Math.min(max,v)); inp.value=v2;
  }
  function populateBetSelect(max){
    var sel=qs('#betSelect'); var prev=sel.value; sel.innerHTML='';
    var up=(typeof max==='number'&&max>=0)?max:20;
    for(var i=0;i<=up;i++){ var o=document.createElement('option'); o.value=i; o.textContent=i; sel.appendChild(o); }
    if(prev && parseInt(prev,10)<=up){ sel.value=prev; }
  }
  ...
  function setupBettingRound(){
    ...
    gameRef.transaction(function(data){
      ...
      if(typeof data.resolverCid==='undefined') data.resolverCid=null;
      return data;
    });

    if(!betUIInited){
      populateBetSelect(undefined);
      qs('#betSelect').value='0'; qs('#betInput').value='0';
    ...
    function echo(){
      var d=lastSnapshot||{};
      var mine=(d.bets && typeof d.bets[CLIENT_ID]==='number')? d.bets[CLIENT_ID] : null;
      qs('#betEcho').innerText=(typeof mine==='number')? ('Puntata salvata sul server: '+mine) : '';
    }

    db.ref('games/'+currentGameId).on('value', function(s){
      var d=s.val()||{};
      lastSnapshot=d; showDiagGame(d);
      if(d.phase!=='betting') return;

      var id1=d.player1 && d.player1.clientId;
      var id2=d.player2 && d.player2.clientId;
      var both=!!(id1 && id2);
      if(both && !roleKnown){
        myRole=(CLIENT_ID===id1)?'p1':'p2';
        roleKnown=true; setBetControlsEnabled(true);
        if(!d.bets || typeof d.bets[CLIENT_ID] !== 'number'){
          db.ref('games/'+currentGameId+'/bets/'+CLIENT_ID).set(0);
        }
      } else if(!both){
        setBetControlsEnabled(false);
      }
      ...
      if(typeof d.betDeadlineAt==='number'){
        ...
        function updateTimer(){
          var rem=Math.max(0, Math.floor((localDeadline - Date.now())/1000));
          qs('#betTimeLeft').textContent=rem;
          if(rem===0){ setBetControlsEnabled(false); }
        }
        updateTimer();
        ...
        reconcileInterval=setInterval(function(){
          var now=Date.now();
          var overdue=(typeof lastSnapshot.betDeadlineAt==='number') && now>(lastSnapshot.betDeadlineAt+500);
          if(overdue && lastSnapshot.phase==='betting' && !lastSnapshot.roundResolved){
            requestResolve();
          } else if(lastSnapshot.phase!=='betting'){
            clearInterval(reconcileInterval);
          }
        }, 1000);
      }

      var myTok=(myRole==='p1')? d.tokens_p1 : d.tokens_p2;
      var oppTok=(myRole==='p1')? d.tokens_p2 : d.tokens_p1;
      if(typeof myTok==='number'){
        qs('#myTokens').textContent=myTok;
        var inp=qs('#betInput'); var sel=qs('#betSelect');
        var typing=document.activeElement===inp;
        if(currentMaxTok!==myTok){
          currentMaxTok=myTok; var prevSel=sel.value; inp.max=myTok;
          if(!typing) clampBetInput(myTok);
          populateBetSelect(myTok);
          if(!typing){ if(prevSel) sel.value=prevSel; else sel.value=inp.value; }
        }
      } else { qs('#myTokens').textContent='—'; }
      qs('#turnNo').textContent=d.turn||1;
      if(typeof oppTok==='number' && oppTok<6){ qs('#oppLowWrap').classList.remove('hidden'); } else { qs('#oppLowWrap').classList.add('hidden'); }
    ...
    function saveCurrent(){
      if(!roleKnown) return;
      var val=parseInt(qs('#betInput').value||'0',10) || 0;
      db.ref('games/'+currentGameId+'/bets/'+CLIENT_ID).set(val).then(function(){
        qs('#betStatus').textContent='Puntata salvata ('+val+').';
      });
    }
    function debouncedSave(){ if(debounceTimer) clearTimeout(debounceTimer); debounceTimer=setTimeout(saveCurrent, 120); }

    qs('#betInput').addEventListener('input', function(){
      if(!roleKnown) return;
      var max=parseInt(qs('#betInput').max||'0',10) || 0;
      clampBetInput(max);
      qs('#betSelect').value=String(parseInt(qs('#betInput').value||'0',10) || 0);
      debouncedSave();
    });
    qs('#betSelect').addEventListener('change', function(){
      if(!roleKnown) return;
      var v=parseInt(qs('#betSelect').value||'0',10) || 0;
      qs('#betInput').value=String(v);
      debouncedSave();
    });
    qs('#betZero').addEventListener('click', function(){
      if(!roleKnown) return;
      qs('#betInput').value='0'; qs('#betSelect').value='0';
      debouncedSave();
    });
  }
  ...
  function requestResolve(force){
    ...
    ref.transaction(function(data){
      ...
      if(!force && data.resolverCid && data.resolverCid!==CLIENT_ID) return data;
      if(!data.resolverCid || force) data.resolverCid=CLIENT_ID;
      return data;
    }, function(err, committed, snap){
      if(err || !committed || !snap) return;
      var d=snap.val(); if(d && d.resolverCid===CLIENT_ID){ resolveRound(); }
    });
  }
  ...
  function resolveRound(){
    ...
    ref.transaction(function(data){
      if(!data) return data;
      if(data.phase!=='betting') return data;
      if(data.resolverCid!==CLIENT_ID) return data;
      if(data.roundResolved) return data;
      ...
      var b1=toInt(data.bets && data.bets[id1]);
      var b2=toInt(data.bets && data.bets[id2]);
      var t1=toInt(data.tokens_p1);
      var t2=toInt(data.tokens_p2);
      var cb1=Math.min(b1,t1), cb2=Math.min(b2,t2);
      data.tokens_p1=t1-cb1; data.tokens_p2=t2-cb2;

      var winner=null;
      if(cb1>cb2) winner='p1'; else if(cb2>cb1) winner='p2'; else winner=null;

      if(!data.rounds) data.rounds={};
      var turn=data.turn||1;
  ...
  function showOutcome(){
    db.ref('games/'+currentGameId).on('value', function(s){
      ...
      var last=d.rounds && d.rounds['turn_'+(d.turn||1)];
      if(!last) return;

      var p1name=d.player1 && d.player1.name || 'Giocatore 1';
      var p2name=d.player2 && d.player2.name || 'Giocatore 2';
      ...
      if(last.winner===null){
        ...
        var btn=document.createElement('button'); btn.textContent='Vai al turno successivo';
        ...
        var wName=(last.winner==='p1')? p1name : p2name;
        qs('#winnerInfo').textContent='Ha vinto la puntata: '+wName;
        var btn2=document.createElement('button'); btn2.textContent='Vai alla fase Azione';
  ...
  function showAction(){
    ...
    db.ref('games/'+currentGameId).on('value', function(s){
      ...
      var p1=d.player1 && d.player1.name || 'Giocatore 1';
      var p2=d.player2 && d.player2.name || 'Giocatore 2';
      ...
      qs('#chooseQuestion').onclick=function(){
        guardedTx(function(d){ d.action={ by:d.lastWinner, byClientId:d.winnerClientId, mode:'question', at:Date.now() }; return d; });
      };
      qs('#chooseTruth').onclick=function(){
        guardedTx(function(d){ d.action={ by:d.lastWinner, byClientId:d.winnerClientId, mode:'truth', at:Date.now() }; return d; });
      };
      qs('#proceedNextTurn').onclick=function(){
        guardedTx(function(d){
          d.turn=(d.turn||1)+1;
          d.bets={};
          d.resolverCid=null;
          d.roundResolved=false;
          d.roundInitialized=false;
          d.phase='betting';
          d.advanceAllowed=true;
          d.betDeadlineAt=Date.now()+15000;
          d.action=null;
          return d;
        });
      };
  ...
  function watchPhase(){
    db.ref('games/'+currentGameId).on('value', function(s){
      ...
      if(p==='select'){
        var role=(d.player1 && d.player1.clientId===CLIENT_ID)?'p1' : ((d.player2 && d.player2.clientId===CLIENT_ID)?'p2' : (myRole||'p1'));
        startSelectCards(role);
      } else if(p==='betting'){
        startBetting(); setupBettingRound();
      } else if(p==='outcome'){
        showOutcome();
      } else if(p==='action'){
        showAction();
