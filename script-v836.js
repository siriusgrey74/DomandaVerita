...
function showCreatorStart(){
  ...
  qs('#inviteBtn').addEventListener('click', ()=>{
    ...
    db.ref('games/'+newId).set({ player1:{name, joined:true, clientId:CLIENT_ID}, createdAt:Date.now(), phase:'waiting', version:'8.3.6' });
    ...
    if(typeof d.betDeadlineAt==='number'){
      ...
      watchdogInterval = setInterval(()=>{
        ...
        if(localDeadline && now >= localDeadline){
          const pathReady = whoAmI(d)==='p1' ? 'bet_ready_p1' : 'bet_ready_p2';
...
function submitBet(){
  ...
  const updates = {};
  const me = myRole; // fallback, ma la risoluzione è in whoAmI(d) on read
  updates[`games/${currentGameId}/bet_${me}`] = val;
  updates[`games/${currentGameId}/bet_ready_${me}`] = true;
...
function resolveRound(){
  ...
  ref.transaction(data=>{
    ...
    // Salva anche winnerClientId per gating in Azione
...
function showAction(){
  ...
  ref.on('value', s=>{
    ...
    const me = whoAmI(d);
    myRole = me; // sync cache
    const wCid = d.winnerClientId || winnerClient(d);
    const iAmWinner = (wCid && wCid === (me==='p1'? d.player1?.clientId : d.player2?.clientId));
    ...
    // Bind winner-only controls with server-side guard
    ...
    const guard = (fn)=>{
      return ()=>{
        ...
        db.ref(`games/${currentGameId}`).transaction(data=>{
          ...
          if(data.winnerClientId && data.winnerClientId!==wCid) return data; // qualche altro ha provato a forzare
          return fn(data);
        });
      };
    };

    chooseQuestion.onclick = guard((data)=>{
      data.action = { by:d.lastWinner, byClientId:wCid, mode:'question', at:Date.now() };
      return data;
    });
    chooseTruth.onclick = guard((data)=>{
      data.action = { by:d.lastWinner, byClientId:wCid, mode:'truth', at:Date.now() };
      return data;
    });
    proceedNextTurn.onclick = guard((data)=>{
