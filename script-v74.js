...
function logd(msg){ const d=document.getElementById('diag'); if(d){ d.textContent = '[v7.4] ' + msg; } console.log('[v7.4]', msg); }
...
function startSelectCards(whichRole){
  ...
  qs('#confirmBtn').onclick=()=>{
    ...
    // next phase if both ready
    db.ref(`games/${currentGameId}`).once('value').then(s=>{
      const d=s.val()||{};
      if(d.ready_p1 && d.ready_p2){ db.ref(`games/${currentGameId}/phase`).set('betting'); }
    });
  };
...
function onTimeExpired(){
  ...
  autoComplete();
  lockSelectionUI();
  // abilita conferma se non ancora confermato
  if(!confirmed){
...
function lockSelectionUI(){
  ...
  updateSel(); // aggiorna stato bottone
}
