...
function logd(msg){ const d=document.getElementById('diag'); if(d){ d.textContent = '[v8.3.3] ' + msg; } console.log('[v8.3.3]', msg); }
...
function showCreatorStart(){
  ...
  qs('#inviteBtn').addEventListener('click', ()=>{
    ...
    db.ref('games/'+newId).set({ player1:{name, joined:true}, createdAt:Date.now(), phase:'waiting', version:'8.3.3' });
...
function setupBettingRound(){
  ...
  gameRef.on('value', s=>{
    ...
    if(typeof d.betDeadlineAt==='number'){
      const updateTimer = ()=>{
        const rem = Math.max(0, Math.floor((d.betDeadlineAt - Date.now())/1000));
        qs('#betTimeLeft').textContent = rem;
        if(rem===0 && deadlineInterval){ clearInterval(deadlineInterval); }
      };
      if(deadlineInterval) clearInterval(deadlineInterval);
      updateTimer();
      deadlineInterval = setInterval(updateTimer, 500);
    }
    ...
    // Se deadline scaduta e non ho inviato, invio  il valore attuale (o 0)
    const myReady = myRole==='p1' ? d.bet_ready_p1 : d.bet_ready_p2;
    if(typeof d.betDeadlineAt==='number' && Date.now() >= d.betDeadlineAt && !myReady){
      submitBet(); // invia il valore corrente (di default 0)
    }
    // Se entrambe pronte e non risolto, risolvi
    if(d.bet_ready_p1 && d.bet_ready_p2 && !d.roundResolved){
      resolveRound();
    }
    // Se deadline passata, e almeno una non pronta, forziamo risoluzione trattando mancanti come 0
    if(typeof d.betDeadlineAt==='number' && Date.now() >= d.betDeadlineAt && !d.roundResolved){
      resolveRound();
    }
  });
  ...
  ref.once('value').then(s=>{
    ...
    proceedNextTurn.onclick = ()=>{
      ...
      db.ref(`games/${currentGameId}`).transaction(data=>{
        ...
        data.betDeadlineAt = Date.now() + 15000; // nuova deadline
        return data;
      });
    };
