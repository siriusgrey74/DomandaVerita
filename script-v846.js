...
// Debug bar
if(isDebug()){ const bar=document.getElementById('debugBar'); if(bar) bar.classList.remove('hidden'); }
const frb = document.getElementById('forceResolveBtn');
if(frb) frb.addEventListener('click', ()=> requestResolve(true));
const ssb = document.getElementById('showStateBtn');
if(ssb) ssb.addEventListener('click', async ()=>{
  const s = await db.ref(`games/${currentGameId}`).get();
  const dump = document.getElementById('stateDump');
  if(dump) dump.textContent = JSON.stringify(s.val(), null, 2);
});
...
let deadlineInterval=null, reconcileInterval=null, debounceTimer=null;
...
function showDiagGame(d){
  ...
  const wcid = d?.winnerClientId ?? 'null';
  const res = d?.resolverCid ?? 'null';
  logd(`gid=${currentGameId} me=${wh} cid=${CLIENT_ID} phase=${d?.phase} lastWinner=${lastW} winnerCID=${wcid} resolverCID=${res}`);
}
...
function showCreatorStart(){
  ...
  qs('#inviteBtn').addEventListener('click', ()=>{
    ...
    db.ref('games/'+newId).set({ player1:{name, joined:true, clientId:CLIENT_ID, role:'p1'}, createdAt:Date.now(), phase:'waiting', version:'8.4.6' });
...
function showInvitee(gid){
  ...
  qs('#acceptBtn').addEventListener('click', ()=>{
    ...
    db.ref('games/'+gid+'/player2').set({ name, joined:true, clientId:CLIENT_ID, role:'p2' });
...
function lockGrid(){ qsa('.card').forEach(c=> c.classList.add('disabled')); }
  ...
  const tick=()=>{
    ...
    if(n<=0){
      ...
      db.ref(`games/${currentGameId}`).transaction(data=>{
        ...
        data.bets = {}; // per clientId
        data.resolverCid = null; // leader del round
...
function setupBettingRound(){
  ...
  const echo = ()=>{
    ...
    const mine = d.bets && typeof d.bets[CLIENT_ID] === 'number' ? d.bets[CLIENT_ID] : null;
  ...
  db.ref(`games/${currentGameId}`).on('value', s=>{
    ...
    const id1 = d.player1?.clientId;
    const id2 = d.player2?.clientId;
    const bothKnown = !!(id1 && id2);
    if(bothKnown && !roleResolved){
      if(CLIENT_ID===id1) myRole='p1'; else if(CLIENT_ID===id2) myRole='p2';
      roleResolved = true;
      setBetControlsEnabled(true);
      if(typeof d.bets?.[CLIENT_ID] !== 'number'){
    ...
    if(typeof d.betDeadlineAt==='number'){
      ...
      // Retry resiliente: se scaduti i 15s e siamo ancora in betting, prova a risolvere ogni 1s
      if(reconcileInterval) clearInterval(reconcileInterval);
      reconcileInterval = setInterval(()=>{
        const now = Date.now();
        const overdue = (typeof lastSnapshot?.betDeadlineAt==='number') && now > (lastSnapshot.betDeadlineAt + 500);
        if(overdue && lastSnapshot?.phase==='betting' && !lastSnapshot?.roundResolved){
          requestResolve(); // election + resolve
  ...
  async function saveCurrent(){
    ...
    await db.ref(`games/${currentGameId}/bets/${CLIENT_ID}`).set(val);
...
// Resolver election + resolve
function requestResolve(force=false){
  const ref = db.ref(`games/${currentGameId}`);
  ref.transaction(data=>{
    if(!data) return data;
    if(data.phase!=='betting') return data;
    if(data.roundResolved) return data;
    if(!force && data.resolverCid && data.resolverCid!==CLIENT_ID){
      return data;
    }
    if(!data.resolverCid){
      data.resolverCid = CLIENT_ID;
    } else if(force){
      data.resolverCid = CLIENT_ID; // forzatura in debug
    }
    return data;
  }, (err, committed, snap)=>{
    if(err || !committed || !snap) return;
    const d = snap.val();
    if(d && d.resolverCid === CLIENT_ID){
      resolveRound();
    }
  });
}
...
function resolveRound(){
  ...
  ref.transaction(data=>{
    ...
    const id1 = data.player1?.clientId || null;
    const id2 = data.player2?.clientId || null;
    const b1 = toInt(data.bets?.[id1]);
    const b2 = toInt(data.bets?.[id2]);
...
function showAction(){
  ...
  ref.on('value', s=>{
    ...
    const iAmWinner = d.winnerClientId && d.winnerClientId === CLIENT_ID;
