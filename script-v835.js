...
function showDiagGame(d){
  const who = (CLIENT_ID===d?.player1?.clientId)?'p1':(CLIENT_ID===d?.player2?.clientId?'p2':'?');
  const lastW = d?.lastWinner ?? 'null';
  logd(`gid=${currentGameId} role=${myRole} resolved=${who} phase=${d?.phase} lastWinner=${lastW}`);
}

// Routing decision (initial guess, will be corrected after binding clientId)
...
function showCreatorStart(){
  ...
  qs('#inviteBtn').addEventListener('click', ()=>{
    ...
    db.ref('games/'+newId).set({ player1:{name, joined:true, clientId:CLIENT_ID}, createdAt:Date.now(), phase:'waiting', version:'8.3.5' });
...
function attachCreatorListener(gid){
  ...
  gameListener = db.ref('games/'+gid).on('value', s=>{
    ...
    // Se questo client non risulta bound come p1, correggi myRole
    if(d.player1?.clientId === CLIENT_ID && myRole!=='p1'){ myRole='p1'; logd('role corrected to p1 by clientId'); }
...
function showInvitee(gid){
  ...
  db.ref('games/'+gid).on('value', s=>{
    const d=s.val()||{};
    showDiagGame(d);
    // Correggi ruolo se questo client corrisponde a p2
    if(d.player2?.clientId === CLIENT_ID && myRole!=='p2'){ myRole='p2'; logd('role corrected to p2 by clientId'); }
    ...
    if(typeof d.betDeadlineAt==='number'){
      ...
      watchdogInterval = setInterval(()=>{
        ...
        if(localDeadline && now >= localDeadline){
          const pathReady = myRole==='p1' ? 'bet_ready_p1' : 'bet_ready_p2';
...
function showAction(){
  ...
  ref.once('value').then(s=>{
    const d=s.val()||{};
    showDiagGame(d);
    const winner = d.lastWinner || null;
    // Calcola il ruolo reale di questo client in base ai clientId
    const resolvedRole = (CLIENT_ID===d?.player1?.clientId)?'p1':((CLIENT_ID===d?.player2?.clientId)?'p2':myRole);
    myRole = resolvedRole; // correzione definitiva
    const myIsWinner = (winner===myRole);
    const p1name = (d.player1 && d.player1.name) ? d.player1.name : 'Giocatore 1';
    const p2name = (d.player2 && d.player2.name) ? d.player2.name : 'Giocatore 2';
    qs('#actionForWinner').classList.toggle('hidden', !myIsWinner);
    qs('#actionForLoser').classList.toggle('hidden', myIsWinner);
    // Il pulsante per andare al prossimo turno è abilitato SOLO al vincitore
    qs('#proceedNextTurn').disabled = !myIsWinner;

    if(winner===null){
      qs('#actionStatus').textContent = "Pareggio: nessuna azione. Il creatore può riavviare la puntata.";
      return;
    }
    const wName = winner==='p1'? p1name : p2name;
    if(myIsWinner){
    ...
    chooseQuestion.onclick = ()=>{
      if(!myIsWinner) return;
      db.ref(`games/${currentGameId}/action`).set({ by:winner, mode:'question', at:Date.now() });
      qs('#actionStatus').textContent = "Hai scelto: Domanda (placeholder).";
    };
    chooseTruth.onclick = ()=>{
      if(!myIsWinner) return;
      db.ref(`games/${currentGameId}/action`).set({ by:winner, mode:'truth', at:Date.now() });
      qs('#actionStatus').textContent = "Hai scelto: Verità (placeholder).";
    };

    proceedNextTurn.onclick = ()=>{
      if(!myIsWinner) return;
      db.ref(`games/${currentGameId}`).transaction(data=>{
        if(!data) return data;
        ...
        data.roundInitialized = false; // abilita +2 alla prossima puntata
        data.phase = 'betting';
    ...
    // Reflect action selection to loser side
    db.ref(`games/${currentGameId}/action`).on('value', snap=>{
      ...
      if(myIsWinner){
        ...
        const wName2 = winner==='p1'? p1name : p2name;
        qs('#actionStatus').textContent = wName2 + " ha scelto: " + modeTxt + " (placeholder).";
      }
    });
  });
}

// Global phase watcher with guard
function watchPhase(){
  db.ref(`games/${currentGameId}`).on('value', s=>{
    const d=s.val()||{};
    const p=d.phase;
