...
function logd(msg){ const d=document.getElementById('diag'); if(d){ d.textContent = '[v8.3] ' + msg; } console.log('[v8.3]', msg); }
...
function populateBetSelect(max){
  ...
  sel.innerHTML = '';
  // Fallback se max non disponibile ancora: 0..20
...
function setupBettingRound(){
  ...
  // Ensure controls populated immediately
...
function resolveRound(){
  ...
  ref.transaction(data=>{
    ...
    data.phase = 'outcome';
    data.lastWinner = winner; // save winner for action phase
...
function showOutcome(){
  ...
  ref.on('value', s=>{
    ...
    // Niente auto-timer: mostriamo pulsante per procedere alla Fase Azione
    const post = qs('#postOutcome');
    post.innerHTML = "";
    const btn = document.createElement('button');
    btn.textContent = "Vai alla fase Azione";
    btn.onclick = ()=> db.ref(`games/${currentGameId}/phase`).set('action');
...
function showAction(){
  ...
  ref.once('value').then(s=>{
    ...
    if(winner===null){
      // Pareggio: per ora saltiamo direttamente al prossimo betting (come fallback)
      qs('#actionStatus').textContent = "Pareggio: nessuna azione. Si passerà al prossimo turno.";
      setTimeout(()=> db.ref(`games/${currentGameId}/phase`).set('betting'), 2000);
      return;
    }
