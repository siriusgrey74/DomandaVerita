...
function logd(msg){ const d=document.getElementById('diag'); if(d){ d.textContent = '[v8.1] ' + msg; } console.log('[v8.1]', msg); }
...
function setupBettingRound(){
  ...
  gameRef.on('value', s=>{
    ...
    const inp = qs('#betInput');
    if(typeof myTok==='number'){ inp.max = myTok; clampBetInput(myTok); }
  });
...
function resolveRound(){
  ...
  ref.transaction(data=>{
    ...
    else winner = null; // pareggio: nessun vincitore
...
function showOutcome(){
  ...
  ref.on('value', s=>{
    ...
    if(last){
      ...
      qs('#nextPhaseHint').textContent = "Prossimo: fase 'azione' (placeholder), poi nuovo turno.";
    }
  });
}
...
function watchPhase(){
  db.ref(`games/${currentGameId}/phase`).on('value', s=>{
    ...
    if(p==='betting'){
      ...
      qs('#startBettingCountdown').classList.add('hidden');
      startBetting();
      setupBettingRound();
    } else if(p==='outcome'){
      showOutcome();
      // Prepare next turn increment and cleanup for next betting phase
      db.ref(`games/${currentGameId}`).transaction(data=>{
        ...
        // In attesa di "azione", per test loopiamo a betting
        data.phase = 'betting';
        return data;
      });
    }
