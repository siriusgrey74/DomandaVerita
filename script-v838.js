...
function showCreatorStart(){
  ...
  qs('#inviteBtn').addEventListener('click', ()=>{
    ...
    db.ref('games/'+newId).set({ player1:{name, joined:true, clientId:CLIENT_ID}, createdAt:Date.now(), phase:'waiting', version:'8.3.8' });
...
function setupBettingRound(){
    ...
    if(typeof d.betDeadlineAt==='number'){
      ...
      watchdogInterval = setInterval(()=>{
        ...
        if(localDeadline && now >= localDeadline){
          resolveRound(); // non dipende più da 'ready', legge i valori bet_current_*
  ...
  // Interazioni: ogni modifica salva SUBITO su DB (bet_current_*)
  const saveCurrent = ()=>{
    const me = myRole || 'p1';
    const val = parseInt(qs('#betInput').value||'0',10) || 0;
    const updates = {};
    updates[`games/${currentGameId}/bet_current_${me}`] = val;
    db.ref().update(updates).then(()=>{
      qs('#betStatus').textContent = "Puntata salvata.";
    });
  };
