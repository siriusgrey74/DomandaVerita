...
function logd(msg){ const d=document.getElementById('diag'); if(d){ d.textContent = '[v8.3.2] ' + msg; } console.log('[v8.3.2]', msg); }
...
function showCreatorStart(){
  ...
  qs('#inviteBtn').addEventListener('click', ()=>{
    ...
    db.ref('games/'+newId).set({ player1:{name, joined:true}, createdAt:Date.now(), phase:'waiting', version:'8.3.2' });
...
function showInvitee(gid){
  ...
  qs('#rejectBtn').addEventListener('click', ()=>{
    // Hotfix: corretto path con parentesi
