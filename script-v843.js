...
function showCreatorStart(){
  ...
  qs('#inviteBtn').addEventListener('click', ()=>{
    ...
    db.ref('games/'+newId).set({ player1:{name, joined:true, clientId:CLIENT_ID, role:'p1'}, createdAt:Date.now(), phase:'waiting', version:'8.4.3' });
...
// Selection (immutato rispetto a v8.4.2)
const suits=['p','c','q','f'];
...
function setupBettingRound(){
  ...
  db.ref(`games/${currentGameId}`).on('value', s=>{
    ...
    // conosciamo entrambi i clientId?
    id1 = d.player1?.clientId || null;
    id2 = d.player2?.clientId || null;
    ...
    if(bothKnown && !roleResolved){
      ...
      // inizializza mia bet
      db.ref(`games/${currentGameId}/bets/${CLIENT_ID}`).transaction(v=> (typeof v==='number'? v : 0));
    } else if(!bothKnown){
      setBetControlsEnabled(false);
    }
...
function showAction(){
  ...
  ref.on('value', s=>{
    ...
    const myCid = CLIENT_ID;
    const iAmWinner = d.winnerClientId && d.winnerClientId === myCid;
