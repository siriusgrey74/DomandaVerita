...
function logd(msg){ const d=document.getElementById('diag'); if(d){ d.textContent = '[v8.3.4] ' + msg; } console.log('[v8.3.4]', msg); }
...
function showCreatorStart(){
  ...
  qs('#inviteBtn').addEventListener('click', ()=>{
    ...
    db.ref('games/'+newId).set({ player1:{name, joined:true}, createdAt:Date.now(), phase:'waiting', version:'8.3.4' });
  ...
  const tick=()=>{
    ...
    if(n<=0){
      ...
      db.ref(`games/${currentGameId}`).transaction(data=>{
        ...
        // imposta deadline e reset puntate
...
function setupBettingRound(){
  ...
  db.ref(`games/${currentGameId}`).on('value', s=>{
    ...
    if(typeof d.betDeadlineAt==='number'){
      ...
      // Avvia watchdog locale che non dipende dagli eventi Firebase
      if(watchdogInterval) clearInterval(watchdogInterval);
      watchdogInterval = setInterval(()=>{
        const now = Date.now();
        if(localDeadline && now >= localDeadline){
          // se non ho inviato, invio 0/valore corrente
          ...
          // prova a risolvere il round (idempotente)
          resolveRound();
    ...
    // Se entrambe pronte e non risolto, risolve
