...
function showCreatorStart(){
  ...
  qs('#inviteBtn').addEventListener('click', ()=>{
    ...
    db.ref('games/'+newId).set({ player1:{name, joined:true, clientId:CLIENT_ID}, createdAt:Date.now(), phase:'waiting', version:'8.4.1' });
...
function setupBettingRound(){
  ...
  qs('#betInput').value = '0';
  setBetControlsEnabled(false); // BLOCCATE finché ruolo non risolto
  roleResolved = false;
  ...
  async function saveCurrent(){
    if(!roleResolved) return; // NON salviamo finché non sappiamo chi siamo
