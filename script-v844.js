...
function showCreatorStart(){
  ...
  qs('#inviteBtn').addEventListener('click', ()=>{
    ...
    db.ref('games/'+newId).set({ player1:{name, joined:true, clientId:CLIENT_ID, role:'p1'}, createdAt:Date.now(), phase:'waiting', version:'8.4.4' });
  ...
  const tick=()=>{
    ...
    if(n<=0){
      ...
      db.ref(`games/${currentGameId}`).transaction(data=>{
        ...
        data.resolverCid = null; // leader del round (verrà settato alla scadenza)
...
// Betting con mappa bets[clientId] e leader unico
...
function setupBettingRound(){
  ...
  db.ref(`games/${currentGameId}`).on('value', s=>{
    ...
    if(bothKnown && !roleResolved){
      // determinazione ruolo una sola volta
      if(CLIENT_ID===id1) myRole='p1';
      else if(CLIENT_ID===id2) myRole='p2';
      roleResolved = true;
      setBetControlsEnabled(true);
      // NON azzerare la mia puntata se esiste già
      const mine = d.bets?.[CLIENT_ID];
    ...
    if(typeof d.betDeadlineAt==='number'){
      ...
      watchdogInterval = setInterval(()=>{
        if(localDeadline && Date.now() >= localDeadline){
          requestResolve(); // usa leader unico
    ...
    if(typeof myTok==='number'){
      ...
      // Aggiorna select SOLO se cambia il massimo consentito
...
// Leader election & resolve
function requestResolve(){
  const ref = db.ref(`games/${currentGameId}`);
  ref.transaction(data=>{
    if(!data) return data;
    if(data.phase!=='betting') return data;
    if(data.resolverCid && data.resolverCid!==CLIENT_ID){
      // qualcun altro è già il leader
      return data;
    }
    if(!data.resolverCid){
      // eleggo me stesso come leader in modo atomico
    ...
    if(!d) return;
    if(d.resolverCid === CLIENT_ID){
      resolveRound(); // solo il leader calcola l'esito
    }
  });
}
