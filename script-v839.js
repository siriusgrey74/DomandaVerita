...
function showCreatorStart(){
  ...
  qs('#inviteBtn').addEventListener('click', ()=>{
    ...
    db.ref('games/'+newId).set({ player1:{name, joined:true, clientId:CLIENT_ID}, createdAt:Date.now(), phase:'waiting', version:'8.3.9' });
...
function setupBettingRound(){
  ...
  db.ref(`games/${currentGameId}`).on('value', s=>{
    ...
    lastSnapshot = d;
    const me = whoAmI(d);
    myRole = me; // *** fix critico: aggiorna il ruolo ad ogni snapshot ***
    showDiagGame(d);
    ...
    if(typeof d.betDeadlineAt==='number'){
      ...
      // Watchdog locale: risolve sempre a 0s
      if(watchdogInterval) clearInterval(watchdogInterval);
      watchdogInterval = setInterval(()=>{
        const now = Date.now();
        if(localDeadline && now >= localDeadline){
          resolveRound(); // legge bet_current_*
  ...
  // Interazioni: ogni modifica salva SUBITO su DB (bet_current_*), calcolando SEMPRE il ruolo dal lastSnapshot
  const resolveRole = ()=>{
    const d = lastSnapshot;
    if(!d) return myRole||'p1';
    return whoAmI(d) || myRole || 'p1';
  };
  const saveCurrent = ()=>{
    const roleNow = resolveRole();
    const val = parseInt(qs('#betInput').value||'0',10) || 0;
    const updates = {};
    updates[`games/${currentGameId}/bet_current_${roleNow}`] = val;
    db.ref().update(updates).then(()=>{
      qs('#betStatus').textContent = `Puntata salvata (${roleNow}=${val}).`;
    });
  };

  // inizializza a 0 il campo corrente del mio ruolo (evita undefined)
  setTimeout(()=>{
    const roleNow = resolveRole();
    const path = `games/${currentGameId}/bet_current_${roleNow}`;
    db.ref(path).transaction(v=> (typeof v==='number'? v : 0) );
  }, 100);
...
function resolveRound(){
  ...
  ref.transaction(data=>{
    ...
    if(data.phase!=='betting') return data;
    // leggi i valori live
    const b1 = Math.max(0, parseInt(data.bet_current_p1||0,10));
    const b2 = Math.max(0, parseInt(data.bet_current_p2||0,10));
    // clamp a token disponibili
    const t1 = data.tokens_p1||0;
    const t2 = data.tokens_p2||0;
...
function showAction(){
  ...
  ref.on('value', s=>{
    ...
    // Winner-only controls with SERVER enforcement
