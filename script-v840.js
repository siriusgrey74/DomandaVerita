...
function showCreatorStart(){
  ...
  qs('#inviteBtn').addEventListener('click', ()=>{
    ...
    db.ref('games/'+newId).set({ player1:{name, joined:true, clientId:CLIENT_ID}, createdAt:Date.now(), phase:'waiting', version:'8.4.0' });
...
function setupBettingRound(){
  ...
  const resolveRoleStrict = async ()=>{
    if(lastSnapshot){
      return whoAmI(lastSnapshot) || myRole || 'p1';
    }
  ...
  const saveCurrent = async ()=>{
    const roleNow = await resolveRoleStrict();
