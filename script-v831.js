...
function logd(msg){ const d=document.getElementById('diag'); if(d){ d.textContent = '[v8.3.1] ' + msg; } console.log('[v8.3.1]', msg); }
...
function showCreatorStart(){
  ...
  qs('#inviteBtn').addEventListener('click', ()=>{
    ...
    db.ref('games/'+newId).set({ player1:{name, joined:true}, createdAt:Date.now(), phase:'waiting', version:'8.3.1' });
...
function setupBettingRound(){
  ...
  gameRef.transaction(data=>{
    ...
    // assegna +2 token solo quando si entra in betting e non è il turno 1 e non ancora applicato
    if(!data.turn){ data.turn = 1; }
    if(typeof data.tokens_p1!=='number') data.tokens_p1 = 10;
    if(typeof data.tokens_p2!=='number') data.tokens_p2 = 10;
...
function resolveRound(){
  ...
  ref.transaction(data=>{
    ...
    data.advanceAllowed = false; // blocca avanzamento automatico
    return data;
  });
}
  ...
  ref.on('value', s=>{
    ...
    btn.onclick = ()=> {
      db.ref(`games/${currentGameId}`).transaction(data=>{
        ...
        data.phase = 'action';
        data.advanceAllowed = false; // si potrà passare al turno successivo solo dopo l'azione del vincitore
        return data;
      });
    };
...
function showAction(){
  ...
  ref.once('value').then(s=>{
    ...
    proceedNextTurn.onclick = ()=>{
      ...
      // Il vincitore avanza il turno: incrementa turn, reset flags, abilita +2 al prossimo ingresso in betting
      db.ref(`games/${currentGameId}`).transaction(data=>{
        if(!data) return data;
        ...
        data.roundInitialized = false; // cosi al prossimo betting si applica il +2
        data.phase = 'betting';
...
function watchPhase(){
  db.ref(`games/${currentGameId}`).on('value', s=>{
    const d=s.val()||{};
    const p=d.phase;
    // Guardia: impedisce salti anomali da outcome->betting senza passare da action
    if(p==='betting' && d.lastPhase==='outcome' && !d.advanceAllowed){
      // rollback forzando action
      db.ref(`games/${currentGameId}/phase`).set('action');
      return;
    }
    // Aggiorna lastPhase
    // (evita loop scrivendo solo se cambia)
    if(d.lastPhase!==p){
